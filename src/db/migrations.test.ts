...
describe('migrations registry', () => {
  it('has all 27 migrations registered', () => {
  ...
  it('last migration is add_source_id_to_key_repair_log', () => {
    const all = registry.getAll();
    const last = all[all.length - 1];
    expect(last.number).toBe(27);
    expect(last.name).toContain('add_source_id_to_key_repair_log');
  });

  it('migrations are sequentially numbered from 1 to 27', () => {
    const all = registry.getAll();
