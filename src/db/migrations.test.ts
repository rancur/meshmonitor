...
describe('migrations registry', () => {
  it('has all 28 migrations registered', () => {
  ...
  it('last migration is add_source_id_to_notifications', () => {
    const all = registry.getAll();
    const last = all[all.length - 1];
    expect(last.number).toBe(28);
    expect(last.name).toContain('add_source_id_to_notifications');
  });

  it('migrations are sequentially numbered from 1 to 28', () => {
    const all = registry.getAll();
